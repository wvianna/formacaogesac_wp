...
jQuery(function()
{
    /*jQuery('#header .menu a').click(function() {
        ...
        jQuery('#content div.middle').addClass('old').fadeOut('fast');
        jQuery('#content').addClass('loading');
        
        jQuery.ajax({
            url : url,
            success : function(response) {
                jQuery('#content div.middle.old').remove();
                
                jQuery('#content').removeClass('loading').append(jQuery(response).find('#content div.middle'));
                
                jQuery('#content div.middle').fadeIn('fast');
            }
        });
    ...
    jQuery('#normaliza_fonte').click(function(){  
        fonte = 12.4167;  
        jQuery('body').css({'font-size' : fonte+'px'});  
    });  
	...
	// Carrega o tamanho do texto
	var cdigital_fontSize = jQuery.cookie('cdigital_fontSize');
	var fontSize = 100;
	
	if(cdigital_fontSize)
	{
		jQuery('#content, div.navFooter').css('fontSize', cdigital_fontSize+'%');
		
		fontSize = parseInt(cdigital_fontSize);
        
        if( fontSize >= 120 ) {
            jQuery('#header div.menu').css('fontSize', '120%');
        }
	}
	
	// Carrega o layout dos diretorios
	var cdigital_directory_layout = jQuery.cookie('cdigital_directory_layout');
	
	if(cdigital_directory_layout)
	{
		if( jQuery('ul.posts').length )
			jQuery('ul.posts').removeClass('two-columns');
	}
	
	// Carrega o contraste
	var cdigital_contrast = jQuery.cookie('cdigital_contrast');
	
	if(cdigital_contrast)
	{
		jQuery('html').addClass(cdigital_contrast);
		jQuery('head').append('<link id="contrast_css" rel="stylesheet" media="screen" href="http://'+ window.location.host +'/wp-content/themes/NewCdigital/global/css/contrast.css" />');
	}

    // Carrega o jquery tabs
    jQuery('div.tabs').tabs();
    var destaque = jQuery('div.hlTabs').tabs().tabs('rotate', 12000, true);

    var totalSize = jQuery('div.hlTabs div.panel').size() -1;
    var next = 0;
    var prev = 0;

    for( $i = 0; $i <= totalSize; $i++ ) {
	prev = ($i != 0) ? $i : totalSize;
	next = ($i != totalSize) ? $i + 2 : 1;

	jQuery( jQuery('div.hlTabs div.panel')[$i] ).append('<a href="#" class="bt_prev" rel="'+ prev +'">Anterior</a>');
	jQuery( jQuery('div.hlTabs div.panel')[$i] ).append('<a href="#" class="bt_next" rel="'+ next +'">Proximo</a>');
    }

    jQuery('.bt_prev, .bt_next', 'div.hlTabs div.panel').click(function() {
	var tab = parseInt(jQuery(this).attr('rel')) - 1;

	console.debug(tab);

	destaque.tabs('select', tab);

	return false;
    });
    
    // Adiciona os bts next e prev
    /*jQuery('div.widget_destaque .bt_prev').unbind().bind('click', function() {
        tabs--;
        
        if( tabs < 0 ) {
	    tabs = jQuery('div.widget_destaque div.panel').length - 1;
        }
        
        jQuery('div.hlTabs').tabs("select", tabs );
        
        return false;
    });
    
    jQuery('div.widget_destaque .bt_next').unbind().bind('click', function() {
        tabs++;
        
        if( tabs > jQuery('div.widget_destaque div.panel').length - 1 ) {
	    tabs = 0;
        }
	...
	// Jquery tooltip
	jQuery('.widget ul#members-list li, .widget ul#groups-list li').tooltip({ 
		delay: 0,
		track: true,
		showURL: false,
		bodyHandler: function() {
			var title = jQuery(this).find('div.item-title a').text();
			var membros = jQuery(this).find('div.item-meta span').text();
	...
	// Controle de fontes
	jQuery('div.acc a').click(function()
	{
		if(jQuery(this).hasClass('fontMinus'))
		{
		    ...
		    jQuery('#content, #footer div.navFooter').css('fontSize', fontSize + '%');

		    if( fontSize == 100 ) {
			jQuery('#header div.menu').css('fontSize', '100%');
		    }
            
		    jQuery.cookie('cdigital_fontSize', fontSize);
		}

		if(jQuery(this).hasClass('fontPlus'))
		{
	...
	jQuery('div#wp-admin-bar li.color a').click(function()
	{
		var $contrast = jQuery(this).attr('id');
		
		if($contrast == 'contrast')
		{
			jQuery('html').attr('class', '');
			jQuery('#contrast_css').remove();
			jQuery.cookie('cdigital_contrast', '');
			
			return false;
		}
		
		jQuery.cookie('cdigital_contrast', $contrast);
		
		jQuery('html').attr('class', '');
		jQuery('html').toggleClass($contrast);
		
		jQuery('head').append('<link id="contrast_css" rel="stylesheet" media="screen" href="http://'+ window.location.host +'/wp/wp-content/themes/NewCdigital/global/css/contrast.css" />');
		
		return false;
	});

	jQuery('div#loading').remove();
	jQuery('div#content, div.navFooter').fadeIn(300);
	
	// Controle de layout dos diretorios
	jQuery('div.layoutControl a').click(function() {
		if(jQuery(this).hasClass('one-column')) {
			jQuery('ul.posts').removeClass('two-columns');
			
			jQuery.cookie('cdigital_directory_layout', 'one-column');
		} else {
			jQuery('ul.posts').addClass('two-columns');
			
			jQuery.cookie('cdigital_directory_layout', '');
		}
	...
	if ( response[0] != "-1" ) {
		jQuery(".widget ul#members-list").fadeOut(200,
			function() {
				...
				jQuery(".widget ul#members-list li").tooltip({ 
					delay: 0,
					track: true,
					showURL: false,
					bodyHandler: function() {
	...
	if ( response[0] != "-1" ) {
		jQuery(".widget ul#groups-list").fadeOut(200,
			function() {
				...
				jQuery(".widget ul#groups-list li").tooltip({
					delay: 0,
					track: true,
					showURL: false,
					bodyHandler: function() {
